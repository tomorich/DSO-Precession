...
Vec3 FullSystem::calcuW () //!姿态差分计算角速度
{
    //if (allFrameHistory.size()>10){

        Mat33 SW, SW2;
        Vec3 WW, WW2, Rt;
        double aaa;
        SE3 camToWorld1=allFrameHistory[allFrameHistory.size()-2]->camToWorld, camToWorld0=allFrameHistory[allFrameHistory.size()-3]->camToWorld, \
        camToWorld2=allFrameHistory[allFrameHistory.size()-1]->camToWorld;
       // std::cout<<camToWorld1.rotationMatrix()<<"-----"<<camToWorld0.rotationMatrix()<<std::endl;
        Eigen::Quaterniond qq,q,qqq;
        Rt = camToWorld1.rotationMatrix()*Vec3 (1,1,1);
        SW=(camToWorld2.rotationMatrix()-camToWorld0.rotationMatrix())*0.5 * camToWorld1.rotationMatrix().inverse()*30;
        qq.x()= (camToWorld1.so3().unit_quaternion().x()-camToWorld0.so3().unit_quaternion().x());
        qq.y()= (camToWorld1.so3().unit_quaternion().y()-camToWorld0.so3().unit_quaternion().y());
        qq.z()= (camToWorld1.so3().unit_quaternion().z()-camToWorld0.so3().unit_quaternion().z());
        qq.w()= (camToWorld1.so3().unit_quaternion().w()-camToWorld0.so3().unit_quaternion().w());
        q=camToWorld1.so3().unit_quaternion();
        qqq=qq*q;
        WW2(0) = qqq.x()*2*30;
        WW2(1) = qqq.y()*2*30;
        WW2(2) = qqq.z()*2*30;
        //std::cout<< qq.x() <<"      "<<q.x()<<std::endl;
        //WW2=2*qq*q.conjugate();
        WW(0)=SW(2,1);
        WW(1)=SW(0,2);
        WW(2)=SW(1,0);
        //*WW1 = WW;
        double LW=sqrt(WW(0)*WW(0)+WW(1)*WW(1)+WW(2)*WW(2));
        double LW2=sqrt(WW2(0)*WW2(0)+WW2(1)*WW2(1)+WW2(2)*WW2(2));
        // std::cout<<WW<<" Rmat  "<<LW<<std::endl;
        //std::cout<<SW<<std::endl;
        //std::cout<<WW2<<" qua  "<<LW2<<"   "<<allFrameHistory.size()<<std::endl;

        //std::cout<<2*(allFrameHistory[allFrameHistory.size()-1]->camToWorld.unit_quaternion()-allFrameHistory[allFrameHistory.size()-2]->camToWorld.unit_quaternion())
        //*allFrameHistory[allFrameHistory.size()-1]->camToWorld.so3().unit_quaternion().conjugate()<<std::endl;
        //sleep(3);
...
/*SE3 model()
    {
        if (full->allFrameHistory.size()>30){
            double dt = 1/30, w0=0.1043, r0=1.03, tnow=full->allFrameHistory[full->allFrameHistory.size()-1]->timestamp;
            Mat33 dR = Eigen::AngleAxisd (w0*tnow, Vec3(-0.0031,-0.1042,-0.0038)).toRotationMatrix();
            Vec3 dtt(r0*sin(w0*tnow*0.5)*cos(w0*tnow*0.5)*2,0,r0*sin(w0*tnow*0.5)*sin(w0*tnow*0.5)*2);
            //dR = dR * allFrameHistory[allFrameHistory.size()-2]->camToWorld.rotationMatrix();
            //dtt = allFrameHistory[allFrameHistory.size()-2]->camToWorld.translation()+dtt;
            SE3 lastFtofh(dR,dtt);
            //fh_2_slast.so3()=lastFtofh;
            //fh->shell->camToWorld=lastFtofh;
            return lastFtofh;
        }
    }*/
...
void FullSystem::printResult(std::string file)
{
	...
	for(FrameShell* s : allFrameHistory)
	{
		...
		myfile  << \
		 //s->incoming_id <<
		  //  " " << s->timestamp << " "<<
		// s->camToWorld.translation().transpose()*(s->camToWorld.rotationMatrix().inverse()-SE3().rotationMatrix()).inverse()
		//	<<"\n " << (s->camToWorld.rotationMatrix().inverse()-SE3().rotationMatrix()).inverse()
	...
	//myfile << allFrameHistory.size()<< "\n";
	myfile.close();
    //std::cout<< SE3().rotationMatrix() << std::endl;
}
...
//	printf("size: %d \n", lastF_2_fh_tries.size());
	...
	else
	{
        ...
        //FrameShell* slast = allFrameHistory[1];   // 上一帧
        //FrameShell* sprelast = allFrameHistory[2];  // 大上一帧
        //sleep(1);
...
/*double dt = fh->shell->timestamp - lastF->shell->timestamp;
Mat33 dR = Eigen::AngleAxisd (0.1043*dt, Vec3(-0.0031,-0.1042,-0.0038)).toRotationMatrix();
Sophus::SO3d lastFtofh(dR);
fh_2_slast.so3()=lastFtofh; */
//! 尝试将自转模型加进去，但是发现这里相对位姿其实还有位移，所以只用旋转信息貌似不行
      /*  double dt = 1/30, w0=0.1, r0=1.0, tnow=fh->shell->timestamp, tf=lastF->shell->timestamp;
        Mat33 dR = Eigen::AngleAxisd (w0*(tnow-tf), Vec3(0,-1,0)).toRotationMatrix();
        Vec3 dtt(r0*(sin(w0*tnow*0.5)*cos(w0*tnow*0.5)-sin(w0*tf*0.5)*cos(w0*tf*0.5))*2,0,
                 r0*(sin(w0*tnow*0.5)*sin(w0*tnow*0.5)-sin(w0*tf*0.5)*sin(w0*tf*0.5))*2);
        SE3 lastf2fh(dR,dtt);*/
//! zhangdong model
        double dt = 1/30, w0=0.1960, r0=0.9539, tnow=fh->shell->timestamp, tf=lastF->shell->timestamp, wh=0.1047, wn=0.1047, theta= 0.6981;
        Mat33 dR1 = Eigen::AngleAxisd (wh*tnow, Vec3(0,0,1)).toRotationMatrix()*Eigen::AngleAxisd (theta, Vec3(1,0,0)).toRotationMatrix()
        *Eigen::AngleAxisd (wn*tnow, Vec3(0,0,1)).toRotationMatrix();
        Mat33 dR2 = Eigen::AngleAxisd (wh*tnow, Vec3(0,0,1)).toRotationMatrix()*Eigen::AngleAxisd (theta, Vec3(1,0,0)).toRotationMatrix()
        *Eigen::AngleAxisd (wn*tnow, Vec3(0,0,1)).toRotationMatrix();
        Vec3 dtt1,dtt2,dtt(-0.01656,-0.023208,0.875312);
        dtt1 = dR1*(-dtt)+dtt;
        dtt2 = dR2*(-dtt)+dtt;
        SE3 lastf2fh(dR2.inverse()*dR1,dtt2-dtt1);
		//! 尝试不同的运动
		// get last delta-movement.
		lastF_2_fh_tries.push_back(fh_2_slast.inverse() * lastF_2_slast);	// assume constant motion.
        lastF_2_fh_tries.push_back(lastf2fh);
        lastF_2_fh_tries.push_back(lastf2fh.inverse());
	...
	int tryIterations=0;
	    ...
	    //std::cout << lastF_2_fh_tries[0].matrix() << std::endl;
        //std::cout << lastF_2_fh_tries.size() << std::endl;
        //sleep(3);
	//! 逐个尝试
...
//[ ***step 2*** ] 尝试不同的运动状态, 得到跟踪是否良好
       ...
       // std::cout<< lastF_2_fh_tries[i].matrix()<< std::endl;
//sleep(1);
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
   ...
   std::cout<<camToWorld1.rotationMatrix()<<"-----"<<camToWorld0.rotationMatrix()<<std::endl;
    Eigen::Quaterniond qq,q,qqq;
     SW=(camToWorld1.rotationMatrix()-camToWorld0.rotationMatrix()) * camToWorld1.rotationMatrix().inverse()*30;
     qq.x()= (camToWorld1.so3().unit_quaternion().x()-camToWorld0.so3().unit_quaternion().x());
     qq.y()= (camToWorld1.so3().unit_quaternion().y()-camToWorld0.so3().unit_quaternion().y());
     qq.z()= (camToWorld1.so3().unit_quaternion().z()-camToWorld0.so3().unit_quaternion().z());
     qq.w()= (camToWorld1.so3().unit_quaternion().w()-camToWorld0.so3().unit_quaternion().w());
...
//[ ***step 6*** ] 判断是否插入关键帧
        ...
        Mat33 re=fh->shell->camToWorld.rotationMatrix()+allFrameHistory[allFrameHistory.size()-3]->camToWorld.rotationMatrix()-
        2*allFrameHistory[allFrameHistory.size()-2]->camToWorld.rotationMatrix();
		Vec3 te=fh->shell->camToWorld.translation()+allFrameHistory[allFrameHistory.size()-3]->camToWorld.translation()-
                2*allFrameHistory[allFrameHistory.size()-2]->camToWorld.translation();
		//std::cout<<re<<std::endl;

//fh->fir2fh=fh->shell->camToWorld;

        SE3 first2fh;
// zizhuan
       /* if (allFrameHistory.size()>10){
            double dt = 1/30, w0=0.1052, r0=0.8909, tnow=fh->shell->timestamp;
            Mat33 dR = Eigen::AngleAxisd (w0*tnow, Vec3(0.0327,-0.9989,-0.0332)).toRotationMatrix();
            Mat33 dR1=fh->shell->camToWorld.rotationMatrix();
            Mat33 ra=dR1*dR.inverse();
            Vec3 dtt(r0*sin(w0*tnow*0.5)*cos(w0*tnow*0.5)*2,0,r0*sin(w0*tnow*0.5)*sin(w0*tnow*0.5)*2);
            //dR =fh->shell->camToWorld.rotationMatrix();
            //dtt(0)=fh->shell->camToWorld.translation().x();
            //dtt(1)=0;
            //dtt(2)=fh->shell->camToWorld.translation().z();
            //Mat33  dR = Eigen::AngleAxisd (0, Vec3(-0.0031,-0.1042,-0.0038)).toRotationMatrix();
            //Vec3 dtt(w0*tnow, 0, 0);
            //dR = dR * allFrameHistory[allFrameHistory.size()-2]->camToWorld.rotationMatrix();
            //dtt = allFrameHistory[allFrameHistory.size()-2]->camToWorld.translation()+dtt;
            SE3 lastFtofh(dR,dtt);
            fh->shell->fir2fh=lastFtofh;
            //fh->shell->fir2fh=fh->shell->camToWorld;
            std::cout<< ra<<std::endl;
            // fh->shell->camToWorld=lastFtofh;
        }*/
//zhangdong
        if (allFrameHistory.size()>30){
            double dt = 1/30, w0=0.1960, r0=0.9539, tnow=fh->shell->timestamp,x0= -0.01656,y0=-0.023208,z0=0.873512, wh=0.1056, wn=0.1031, theta= 0.7340,
            wh0=0.1047,wn0=0.1047,theta0=0.6981;
            Mat33 dR = fh->shell->camToWorld.rotationMatrix();
            Mat33 dR_last = allFrameHistory[allFrameHistory.size()-31]->camToWorld.rotationMatrix();
            Vec3 dtt;
            dtt = fh->shell->camToWorld.translation();
            double ds,dx,dy,dz,ds1;
            dx =dtt(0);
            dy =dtt(1);
            dz =dtt(2);
            ds =0.9089/((dtt(0)-x0)*(dtt(0)-x0)+(dtt(1)-y0)*(dtt(1)-y0)+(dtt(2)-z0)*(dtt(2)-z0));
            /*dtt(0)=(dtt(0)-x0)*sqrt(ds)+x0;
            dtt(1)=(dtt(1)-y0)*sqrt(ds)+y0;
            dtt(1)=(dtt(1)-z0)*sqrt(ds)+z0;*/
            ds1 = 0.9089/((dtt(0)-x0)*(dtt(0)-x0)+(dtt(1)-y0)*(dtt(1)-y0)+(dtt(2)-z0)*(dtt(2)-z0));
            //std::cout<< ds << "ssss" << ds1<< std::endl<<dtt <<"ssss"<< dx<<"  "<<dy<<"  "<<dz<<std::endl;
            SE3 lastFtofh(dR,dtt);
Mat33 r1 = Eigen::AngleAxisd (wh0*tnow, Vec3(0,0,1)).toRotationMatrix();
Mat33 r2 = Eigen::AngleAxisd (wn0*tnow, Vec3(0,0,1)).toRotationMatrix();
Mat33 rtheta = Eigen::AngleAxisd (theta0, Vec3(1,0,0)).toRotationMatrix();
Mat33 ra0= Eigen::AngleAxisd (0.02, Vec3(0,0,1)).toRotationMatrix()*Eigen::AngleAxisd (-0.02 ,Vec3(0,1,0)).toRotationMatrix()
        *Eigen::AngleAxisd (0.89, Vec3(1,0,0)).toRotationMatrix();
Mat33 rbz0=Eigen::AngleAxisd(-0.01,Vec3(0,0,1)).toRotationMatrix();
Mat33 rb0= Eigen::AngleAxisd (-17.5572, Vec3(0,0,1)).toRotationMatrix()*Eigen::AngleAxisd (284.268 ,Vec3(0,1,0)).toRotationMatrix()
                       *Eigen::AngleAxisd (-0.303161, Vec3(1,0,0)).toRotationMatrix();
            Mat33 r1_last = Eigen::AngleAxisd (wh*(tnow-1), Vec3(0,0,1)).toRotationMatrix();
            Mat33 r2_last = Eigen::AngleAxisd (wn*(tnow-1), Vec3(0,0,1)).toRotationMatrix();
Mat33 ra, rb = dR*r2.inverse();
SO3 SO3_r1(r1),SO3_rb(rb);
Vec3 so3p1 = SO3_r1.log(), so3p2 = SO3_rb.log();
Mat33 rp1, rp2;
rp1 = SO3::hat(so3p1);
rp2 = SO3::hat(so3p2);
//ra = so3p2*so3p1.inverse();
//ra = dR*r2.inverse()*rtheta.inverse()*r1.inverse();
ra = r1.inverse()*rtheta.inverse()*r2.inverse()*dR;
Sophus::Quaterniond qa,q1=Sophus::Quaterniond(r1),q2=Sophus::Quaterniond(r2),qr=Sophus::Quaterniond(dR),qb,qb1=Sophus::Quaterniond(rb);
double xa,ya,za,wa;
qb=qr*q2.inverse();

//Mat33 ra_err=raa[raa.size()]-raa[raa.size()-1];
Mat33 dR2 = ra*r1*ra.inverse()*r2;
            Mat33 dR1 = r1*rtheta*r2,dR1_last=r1_last*rtheta*r2_last,R11,R12,I;
            Vec3 t11;
            raa.push_back(dR1);
            rbb.push_back(dR);
            //dR1_last(2,2)=-0.0001;
R11=dR1.inverse()-dR1_last.inverse();
R12=dR-dR_last;
ra = dR*dR1.inverse();
t11=(I-dR).inverse()*fh->shell->camToWorld.translation();

            //std::cout<< Eigen::AngleAxisd (wh*tnow, Vec3(0,0,1)).toRotationMatrix()<<std::endl<<Eigen::AngleAxisd (theta, Vec3(1,0,0)).toRotationMatrix()<<
            //std::endl<<Eigen::AngleAxisd (wn*tnow, Vec3(0,0,1)).toRotationMatrix();
            Vec3 dtt1,dtt2,dtt0(-0.01656,-0.023208,0.875312),dtt00(0.001,0.8758,0.001);
            dtt1 = ra0*dR1*(-dtt00)+dtt0;
            ttt.push_back(dR*(-dtt0)+dtt0);
            //ttt.push_back(dtt1);
            //dtt1=dR*(-dtt0)+dtt0;
            t11=rb0*dtt0-dtt00;
            std::cout<<t11<<"----"<<std::endl;
//std::cout<<"-------"<<std::endl<<dR1<<std::endl<<"-------"<<std::endl<<dR<<std::endl;
//sleep(1);
            //SE3 lastFtofh2(dR1,dtt1);
            SE3 lastFtofh2(dR,dtt1);
            //fh->shell->fir2fh=lastFtofh2;
             fh->shell->fir2fh=fh->shell->camToWorld;
             //fh->shell->camToWorld=lastFtofh;
        }
        else {fh->shell->fir2fh=fh->shell->camToWorld;}

        //Vec3 tte=fh->shell->fir2fh.translation()-fh->shell->camToWorld.translation();
        Vec3 tte=fh->shell->fir2fh.translation()-allFrameHistory[allFrameHistory.size()-2]->camToWorld.translation();
        if (tte.norm()>0.01)
        {
            fh->shell->camToWorld=fh->shell->fir2fh;
            fh->shell->ismapping=false;
            //needToMakeKF= false;
        ...
        /*if(abs(te.y())>0.08){
           fh->shell->fir2fh.translation().y()=0;
            //delete fh;
            //return;
     needToMakeKF= false;
     }*/
        Mat44 err;
        err = fh->shell->fir2fh.matrix()-fh->shell->camToWorld.matrix();
        //std::cout<< fh->shell->fir2fh.matrix() << std::endl;
        //sleep(0.4);
        //td::cout<< fh->shell->camToWorld.rotationMatrix() << std::endl;

        outputWrapper[0]->first2fh=fh->shell->fir2fh;
        //outputWrapper[0]->ismapping=fh->shell->ismapping;
        //std::cout<<"a\n"<< first2fh.matrix()<<std::endl;
...
Vec3 FullSystem::print() {
    return ttt[ttt.size()-1];
};
struct CURVE_FITTING_COST {

        CURVE_FITTING_COST( const Mat33 &x,const Mat33 &y) : _x(x), _y(y) {}

        // 残差的计算
        template<typename T>
        bool operator()(
                const T * abc, // 模型参数，有3维
                T *residual) const {
            typedef Eigen::Matrix<T,3,3> E_Mat33_T;
            typedef Eigen::Matrix<T,3,1> E_Mat31_T;
            /*E_Mat33_T cH(abc);
            E_Mat33_T dH(def);
            E_Mat33_T cx(T(_x));
            E_Mat33_T cy(T(_y));
            E_Mat33_T re= cH*cx*dH-cy;
            residual[0]=re[0,0]*re[0,0]+ re[1,1]*re[1,1]+re[2,2]*re[2,2]+re[0,1]*re[0,1]+re[0,2]*re[0,2]+re[1,0]*re[1,0]
             +re[2,0]*re[2,0]+re[1,2]*re[1,2]+re[2,1]*re[2,1];
            SymmetricGeometricDistanceTerms<T>(cH,dH,cx,cy,residual[0]);*/
            //E_Mat33_T ra=Eigen::AngleAxisd(abc[0], Vec3(1,0,0)).toRotationMatrix()*Eigen::AngleAxisd(abc[1], Vec3(0,1,0)).toRotationMatrix()
            //        *Eigen::AngleAxisd(abc[2], Vec3(0,0,1)).toRotationMatrix();
            //E_Mat33_T rb=Eigen::AngleAxisd(abc[3], Vec3(1,0,0)).toRotationMatrix()*Eigen::AngleAxisd(abc[4], Vec3(0,1,0)).toRotationMatrix()
            //         *Eigen::AngleAxisd(abc[5], Vec3(0,0,1)).toRotationMatrix();
            E_Mat33_T rax,ray,raz,ra;
            rax << T(1.0), T(0.0), T(0.0),
                    T(0.0), cos(abc[0]), -sin(abc[0]),
                    T(0.0), sin(abc[0]), cos(abc[0]);
            ray << cos(abc[1]),  T(0.0), sin(abc[1]),
                   T(0.0),       T(1.0), T(0.0),
                   -sin(abc[1]), T(0.0), cos(abc[1]);
            raz << cos(abc[2]), -sin(abc[2]), T(0.0),
                    sin(abc[2]), cos(abc[2]), T(0.0),
                    T(0.0),      T(0.0),      T(1.0);
            ra=raz*ray*rax;
            //(T(1.0),T(1.0),T(1.0),T(1.0),T(1.0),T(1.0),T(1.0),T(1.0),T(1.0));
            E_Mat33_T rbx,rby,rbz,rb;
            rbx << T(1.0), T(0.0), T(0.0),
                    T(0.0), cos(abc[3]), -sin(abc[3]),
                    T(0.0), sin(abc[3]), cos(abc[3]);
            rby << cos(abc[4]),  T(0.0), sin(abc[4]),
                    T(0.0),       T(1.0), T(0.0),
                    -sin(abc[4]), T(0.0), cos(abc[4]);
            rbz << cos(abc[5]), -sin(abc[5]), T(0.0),
                    sin(abc[5]), cos(abc[5]), T(0.0),
                    T(0.0),      T(0.0),      T(1.0);
            rb=rbz*rby*rbx;
            E_Mat33_T rbz0;
            //rbz0 << cos(abc[3]), -sin(abc[3]), T(0.0),
            //        sin(abc[3]), cos(abc[3]), T(0.0),
             //       T(0.0),      T(0.0),      T(1.0);
            E_Mat31_T b(T(0.001),T(0.8758),T(0.001)),b0(T(-0.01656),T(-0.023208),T(0.875312));
            E_Mat31_T re;
            //E_Mat31_T b1(abc[3],abc[4],abc[5]);

            //std::cout<<rb1<<std::endl;
            E_Mat33_T xt,yt;
            xt<<  T(_x(0,0)), T(_x(0,1)), T(_x(0,2)),
                    T(_x(1,0)), T(_x(1,1)), T(_x(1,2)),
                    T(_x(2,0)), T(_x(2,1)), T(_x(2,2));
            yt<<  T(_y(0,0)), T(_y(0,1)), T(_y(0,2)),
                    T(_y(1,0)), T(_y(1,1)), T(_y(1,2)),
                    T(_y(2,0)), T(_y(2,1)), T(_y(2,2));
            //E_Mat31_T zt(T(_y(0)),T(_y(1)),T(_y(2)));
            re=yt*b0-ra*xt*b;
            //re = -b-rb*(-b0);
            //re = yt*(-b0)+b0-ra*(xt*(-b)+b);
            T re_max;


            re_max=re(0)*re(0)+re(1)*re(1)+re(2)*re(2);
            /*T re_max=re(0,0);

            for (int i=0;i<3;i++) {
                for (int j = 0; j < 3; j++)
                {
                    if (abs(re(i,j))>abs(re_max))
                    re_max= re(i,j);

                }
            }*/
            residual[0] = re_max; // y-exp(ax^2+bx+c)*/
            return true;
        }

        const Mat33 _x;
        const Mat33 _y;// x,y数据
};



Vec6 FullSystem::calcuRa() {

  double ae=0.8,be=0,ce=0,de=0,ee=0,fe=0;
  int N=raa.size();
    double abc[6] = {ae, be, ce,de,ee,fe};
    ...
    for (int i = 0; i < N; i++) {
        problem.AddResidualBlock(     // 向问题中添加误差项
                ...
                new ceres::AutoDiffCostFunction<CURVE_FITTING_COST, 1, 6>(
                        new CURVE_FITTING_COST(raa[i], rbb[i])
                ),
    ...
    std::cout << std::endl;
    Vec6 abc0;
    abc0(0)=abc[0];abc0(1)=abc[1];abc0(2)=abc[2];abc0(3)=abc[3];abc0(4)=abc[4];abc0(5)=abc[5];
...
//[ ***step 7*** ] 对滑窗内的关键帧进行优化(说的轻松, 里面好多问题)
	...
	if(allKeyFramesHistory.size() <= 4)
	{
        ...
        if(allKeyFramesHistory.size()==2 && rmse > 2000*benchmark_initializerSlackFactor)
		{
			printf("I THINK INITIALIZATINO FAILED! Resetting.\n");
			initFailed=true;
		}
		if(allKeyFramesHistory.size()==3 && rmse > 1300*benchmark_initializerSlackFactor)
		{
			printf("I THINK INITIALIZATINO FAILED! Resetting.\n");
			initFailed=true;
		}
		if(allKeyFramesHistory.size()==4 && rmse > 900*benchmark_initializerSlackFactor)
		{
			printf("I THINK INITIALIZATINO FAILED! Resetting.\n");
			initFailed=true;
		}
