...
void KeyFrameDisplay::setFromF(FrameShell* frame, CalibHessian* HCalib)
{
	...
	camToWorld = frame->fir2fh; //!这句是表达建图是否使用模型轨迹
	needRefresh=true;
...
void KeyFrameDisplay::setFromKF(FrameHessian* fh, CalibHessian* HCalib)
{
	...
	camToWorld = fh->shell->fir2fh;  //!这句是表达建图是否使用模型轨迹
	needRefresh=true;
}
...
void KeyFrameDisplay::drawCam(SE3 first2fh,float lineWidth, float* color, float sizeFactor)
{
	if(width == 0)
		return;
...
//camToWorld=SE3 ();
    ...
    //Sophus::Matrix4f m = first2fh.matrix().cast<float>();
...
void KeyFrameDisplay::drawPC(float pointSize, SE3 first2fh)
{
