...
//if((fh->id<400)||(fh->id>800)&& (fh->id<1000)) {
    //fh->camToWorld=first2fh;
    float blue[3] = {0, 0, 1};
    if (this->settings_showKFCameras) fh->drawCam(first2fh, 1, blue, 0.1);
    ...
    if (full->allFrameHistory.size()>30) {
        double dt = 1 / 30, w0 = 0.1043, r0 = 1.03, tnow = full->allFrameHistory[full->allFrameHistory.size() -
                                                                                 1]->timestamp;
        Mat33 dR = Eigen::AngleAxisd(w0 * tnow, Vec3(-0.0031, -0.1042, -0.0038)).toRotationMatrix();
        Vec3 dtt(r0 * sin(w0 * tnow * 0.5) * cos(w0 * tnow * 0.5) * 2, 0,
                 r0 * sin(w0 * tnow * 0.5) * sin(w0 * tnow * 0.5) * 2);
    ...
    fh->drawPC(1, first2fh);
//}
			}
			if(this->settings_showCurrentCamera) currentCam->drawCam(first2fh,2,0,0.2);
			drawConstraints();
			lk3d.unlock();
		}
...
void PangolinDSOViewer::drawConstraints()
{
	...
	if(settings_showTrajectory)
	{
		...
		for(unsigned int i=0;i<keyframes.size();i++)
		{
            ...
            //glVertex3f((float)first2fh.translation()[0],
            //          (float)first2fh.translation()[1],
            //           (float)first2fh.translation()[2]);
		}
		glEnd();
	}
		...
		CalibHessian* HCalib)
{
	...
	allFramePoses.push_back(frame->fir2fh.translation().cast<float>());
    //allFramePoses.push_back(frame->camToWorld.translation().cast<float>());
	//std::cout<<frame->camToWorld.translation().cast<float>()<<std::endl;
	//sleep(1);
}
